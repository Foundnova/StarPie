...
> **本分支不采用本方案**（2026-09-17 合并时判定）：本文档描述的是 `devplugin` 分支的「统一调用运行时 + 路径模块 + 活动调用租约 + 异步停用状态机」重构。本分支未采纳它 —— 该重构与 `AGENTS.md` §3.7 的顶层类型认领机制、`plugins\` 下的 12 个单动作包在 `PluginHost` 上不可共存，其新增模块 `PluginRuntime` / `PluginPathModules`（915 行、零引用）已随合并后修复删除。保留本文档**仅作设计参考**，插件系统的现行规范见 `AGENTS.md` §3.7。
>
