...
namespace WinPieGestures
{
    ...
    public static class IconHelper
    {
            ...
            string shape, double gap = 0, double cornerRadius = 0)
        {
            ...
            {
                double w = Math.Max(14.0, radialSpan);
                double h = Math.Max(14.0, Math.Min(w * 0.75, tangentialSpan * 0.88));
                double r = Math.Min(h / 2.0, cornerRadius > 0 ? cornerRadius : h / 2.0);
            ...
            {
                double w = Math.Max(14.0, radialSpan);
                double h = Math.Max(14.0, Math.Min(w * 0.85, tangentialSpan * 0.85));
                double r = Math.Max(2.0, cornerRadius > 0 ? cornerRadius : 6.0);
                var rectGeom = new RectangleGeometry(new Rect(lx - w / 2.0, ly - h / 2.0, w, h), r, r);
                rectGeom.Transform = new RotateTransform(midAngle, lx, ly);
                return rectGeom;
            }
