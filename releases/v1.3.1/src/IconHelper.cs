...
namespace WinPieGestures
{
    ...
    public static class IconHelper
    {
            ...
            string shape, double gap = 0, double cornerRadius = 0)
        {
            ...
            else if (shape == "OrganicPetals" || shape == "LiquidDroplets")
            {
                return CreateOrganicPetalGeometry(lx, ly, radialSpan, tangentialSpan, midAngle);
            }
            else if (shape == "ArcTracker" || shape == "MinimalArc")
            {
                return CreateArcTrackerGeometry(cx, cy, startAngle, endAngle, layoutR, radialSpan, gap);
            }
            ...
            else if (shape == "RoundedCapsule" || shape == "FloatingCapsules" || shape == "RoundedRect")
            {
        ...
        private static Geometry CreateOrganicPetalGeometry(double lx, double ly, double radialSpan, double tangentialSpan, double midAngle)
        {
            double length = Math.Max(20.0, radialSpan);
            double width = Math.Max(18.0, Math.Min(length * 0.86, tangentialSpan * 0.92));
            
            double xBase = lx - length / 2.0; // Point towards center core
            double xTip = lx + length / 2.0;  // Outer perimeter bulb
            double yCenter = ly;
            double halfW = width / 2.0;

            var figure = new PathFigure
            {
                StartPoint = new Point(xBase, yCenter),
                IsClosed = true,
                IsFilled = true
            };

            // Top curve towards outer bulb
            figure.Segments.Add(new BezierSegment(
                new Point(xBase + length * 0.18, yCenter - halfW * 0.22),
                new Point(xBase + length * 0.50, yCenter - halfW),
                new Point(xTip - length * 0.22, yCenter - halfW),
                true));

            // Outer bulb smooth crown curve
            figure.Segments.Add(new BezierSegment(
                new Point(xTip, yCenter - halfW * 0.70),
                new Point(xTip, yCenter + halfW * 0.70),
                new Point(xTip - length * 0.22, yCenter + halfW),
                true));
            ...
            figure.Segments.Add(new BezierSegment(
                new Point(xBase + length * 0.50, yCenter + halfW),
                new Point(xBase + length * 0.18, yCenter + halfW * 0.22),
                new Point(xBase, yCenter),
                true));

            var geom = new PathGeometry();
            geom.Figures.Add(figure);
            geom.Transform = new RotateTransform(midAngle, lx, ly);
            return geom;
        }

        private static Geometry CreateArcTrackerGeometry(double cx, double cy, double startAngle, double endAngle, double layoutR, double radialSpan, double gap)
        {
            double trackThickness = Math.Max(12.0, Math.Min(22.0, radialSpan * 0.42));
            double arcInnerR = layoutR - trackThickness / 2.0;
            double arcOuterR = layoutR + trackThickness / 2.0;
            ...
            {
                double angularGap = (gap / layoutR) * (180.0 / Math.PI);
                double span = Math.Abs(endAngle - startAngle);
                if (angularGap < span * 0.7)
                {
                    effStartAngle += angularGap / 2.0;
                    effEndAngle -= angularGap / 2.0;
                }
            ...
            return CreateStandardSectorGeometry(cx, cy, effStartAngle, effEndAngle, arcInnerR, arcOuterR);
        }
