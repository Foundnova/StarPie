...
namespace WinPieGestures
{
    ...
    public static class IconHelper
    {
            ...
            string shape, double gap = 0, double cornerRadius = 0)
        {
            ...
            {
                return CreateArcTrackerGeometry(lx, ly, radialSpan, tangentialSpan, cornerRadius);
            }
        ...
        private static Geometry CreateArcTrackerGeometry(double lx, double ly, double radialSpan, double tangentialSpan, double cornerRadius)
        {
            // Minimalist floating orbital pill / rounded badge centered on the orbit
            double w = Math.Max(28.0, Math.Min(radialSpan * 0.90, tangentialSpan * 0.88));
            double h = Math.Max(26.0, Math.Min(w * 0.92, tangentialSpan * 0.86));
            double r = cornerRadius > 0 ? Math.Min(h / 2.0, cornerRadius + 4.0) : h / 2.0;
            return new RectangleGeometry(new Rect(lx - w / 2.0, ly - h / 2.0, w, h), r, r);
        }
