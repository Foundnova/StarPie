...
1. **删除零引用的孤儿模块** `PluginRuntime.cs`（466 行）与 `PluginPathModules.cs`（449 行）
   - 合并把这两个文件带了进来，但 `PluginCallKind` / `PluginInvocationLease`（定义在那一侧的 `PluginInstance.cs`）没有被带进来 —— 那份 `PluginInstance.cs` 取的是本分支版本，于是 `PluginRuntime.cs` 直接编译失败（`CS0246` × 2）。两个文件在本分支零外部引用，即使编译得过也是 915 行死代码。
   - **删除而非补全**：那两个类型所属的「统一调用运行时 + 路径模块 + 活动调用租约 + 异步停用状态机」重构，与本分支的顶层类型认领机制在 `PluginHost` 上不可共存。将来若要采用它，正确做法是把认领链路重新融合进那套运行时，而不是直接取那一侧的文件。
...
4. **`AGENTS.md` §4「插件系统架构与开发规范」整章重复**
   - 它是那一侧的整章插入：与本分支 §3.7 有 39 行逐字节重复，其余 6 条描述的是本分支未采用的运行时重构。
   - 改写为「§4 插件系统运行时重构（未采用）」并同步目录锚点 —— §3.7 是本分支的现行规范，将来若要采用那套重构的正确路径也记在这里。净删 40 行重复。

5. **`docs/plugin-system-architecture.md` 顶部加状态标注**：该文档描述的是未采用的方案（原文称「已基本完成」），保留**仅作设计参考**。
