...
### 集成 Everything 极速检索核心与「全盘文件与程序秒搜 (Quick Finder)」

1. **官方轻量 Everything SDK 无缝集成**
   - 引入官方 `Everything64.dll` (91 KB, MIT 许可证)，通过 Win32 IPC 毫秒级直接读取系统 Everything 内存索引库，无需常驻后台重型数据库或重复构建索引，实现真正的零额外物理内存开销。
   - 内置智能探测：自适应检测后台 Everything 运行状态；若未安装或未运行，自动降级为本地常用目录极速扫描，并支持一键智能唤起。

2. **「添加程序 / 程序检索器」穿透索引增强**
   - 解决以往免安装、解压即用的「绿色便携程序」在已安装软件列表中找不到的问题。
   - 支持并发穿透全盘检索 `.exe`，自动过滤卸载器、补丁包与临时文件，标记专属「绿色便携」橙色微标，并与本地已安装程序合并去重，支持一键选入轮盘动作槽位。

3. **新增系统控制区「全盘文件与程序秒搜 (Quick Finder / Everything)」功能**
   - 新增轮盘预设动作 `QuickSearch`，支持在鼠标轮盘或快捷手势中一键呼出类似 Spotlight / Listary 的极简悬浮搜索框。
   - **多品类穿透过滤**：支持「全部」、「💻 应用程序」、「📐 机械/CAD (SolidWorks/AutoCAD/STEP/IGES)」、「📄 文档/代码」、「📁 常用目录」、「⚙️ 系统管理」一键切换。
   - **全键盘极速流交互**：支持 `↑`/`↓` 上下浏览，`Enter` 毫秒级打开，`Shift+Enter` 直接在 Windows 资源管理器中高亮定位选中项，`Esc` 退出。
   - **实用右键上下文菜单**：支持立即打开、在资源管理器中定位、以管理员身份运行、复制完整路径、复制文件名。
   - **极致轻量丝滑**：具备防抖异步查询流与毫秒级耗时回显，失焦自动隐藏，完美融合深浅色模式。
