...
5. **黑白名单「➕ 添加进程」点击冗余弹窗修复 (Photo 2 Fix)**：
   - **根因分析**：`AddBlacklistButton_Click` 在输入框为空时原先直接回落调用了 `BrowseBlacklistButton_Click`，导致点击添加进程按钮时弹出了与「🔍 选择应用...」完全相同的应用选择器窗口；
   - **修复调整**：输入框为空时仅将输入焦点定位到进程名输入框 (`NewBlacklistProcessTextBox.Focus()`)，引导用户直接输入；仅在用户点击「🔍 选择应用...」时才呼出应用检索器，功能语义明确清晰。

6. **交互音效突发无声排查与高保真声学校准 (Audio Subsystem Deep Fix)**：
   - **声学能量与声卡唤醒优化**：旧版部分主题（如 Crisp、Minimalist）的 Hover 音效极短（7~10ms）且衰减过陡，极易触发各类声卡 DAC（如 Realtek、蓝牙耳机、笔记本扬声器）的节能休眠与噪声门限（Noise Gate），导致声音输出直接被物理静音截断；现已重构波形合成算法，所有音效均配备 1.2~1.5ms 软起音微窗（杜绝开门爆音），Hover 时长校准为 32~38ms 饱满双频共鸣，确保在任意硬件设备上均清晰、透亮、保真；
   - **对数听觉增益补偿 (`GetAcousticGain`)**：将线性振幅缩放升级为符合人耳听觉曲线的声学增益映射，彻底解决中低音量下音量过小听不清的问题；
   - **非托管内存与回放安全防护**：在 `FreePointers()` 与 `Shutdown()` 释放音频非托管内存指针前，显式调用 `PlaySound(IntPtr.Zero, IntPtr.Zero, 0)` 强制中止 Win32 异步音频流，杜绝底层播放线程读取已释放内存引发的 Access Violation 甚至声卡驱动静默故障；
   - **配置落盘完整性补全**：在 `SyncUiToConfigAndSave()` 中全面补齐 `EnableSoundEffects`、`SoundTheme`、`SoundVolume` 及 5 大细项开关的状态同步，确保用户在控制台中调整的音量、主题与开关随时持久化落盘。
