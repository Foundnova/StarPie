...
1. **全局统一与扇区独立定制分段切换**：
   - 在【图标与排版选项】中新增 `[ 🌐 全局统一排版    🎯 单个扇区独立定制 ]` 分段切换开关；
   - 支持针对特定扇区（槽位 1 ~ N）单独覆盖其**排版模式**（跟随全局 / 图标+文字 / 仅显示图标 / 仅显示文字）、**文字颜色**（Hex 输入、🎨 调色板选择器、🔍 屏幕吸管）、**专属字体**、**独立图标大小**与**独立文字字号**；
   - 提供「重置为全局默认」一键还原功能，兼具全局一致性与个别特殊槽位的极致个性化。
2. **实时交互画布点击联动选中 (Canvas Click-to-Select)**：
   - 右侧实时交互画布（Live Preview）中每个扇区均支持鼠标指针悬停与直接左键点击；
   - 点击任意扇区时，左侧排版面板自动切换至对应扇区槽位并呈现专属天蓝色外发光选中光环，实现直观丝滑的可视化所见即所得调参体验。
