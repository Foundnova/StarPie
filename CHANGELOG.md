...
## [v1.5.6] - 2026-08-31 (运行自定义系统命令 & 终端类型与静默模式选择 & 蜂巢六边形矩阵圆角全量修复)
...
### 🎨 蜂巢六边形平滑倒角算法与 Freezable 渲染修复 (Hexagon Hive Fillet & Freezable Fix)
1. **彻底修复蜂巢六边形矩阵在冻结对象上修改 Transform 导致画布报错空白的 Bug**：
   - 将扇区角度旋转直接融入极坐标 6 顶点三角函数生成链路，避免后置修改已冻结对象；
2. **蜂巢六边形矩阵平滑倒角无级调节**：
   - 无论是主轮盘扇区还是二级蜂窝扇，均支持通过「扇区平滑倒角」滑块从 0px 锐角调节至 20px 胶囊六边形。
