...
## [v1.6.8] - 2026-09-04 (动作画布缩放修复 & 快捷键Pause与搜索 & 独占暂停全局热键 & 侧边栏主题切换 & 贡献者致谢 & 蜂窝扇迟滞保持 & 折叠收纳)
...
3. **重塑「暂停全局热键」为底层独占录入拦截引擎**：
   - 深入利用底层 `WH_KEYBOARD_LL` 钩子优先级高于 Windows 系统热键与所有后台应用程序的物理机制；
   - 点击激活独占录制后，StarPie 底层钩子拦截并吞没（swallow，向系统返回 1）按键事件，**桌面系统（如 Win+D 桌面最小化、Alt+Tab 任务切换、PrintScreen 截屏）及所有其他软件（如微信、QQ等）的全局热键被完全暂时暂停/阻断**；
   - 独占将按键安全送达 StarPie 录制器完成准确录入并自动持久化，录完即刻恢复全局分发，按 Esc 或窗口失焦自动安全退出，录入冲突快捷键不再受任何干扰。
...
### 🤝 系统设置新增开源贡献者致谢折叠卡片
1. **实时同步 GitHub 仓库贡献者**：
   - 在 Tab 4（系统设置）顶部右侧新增现代化折叠致谢卡片 `[ 🤝 开源贡献者致谢 ▼ ]`；
   - 异步对接 GitHub Contributors API (`SoftBlack42/StarPie`)，按贡献量从高到低排列展示圆形头像、贡献者 ID 与提交次数徽标；
   - 支持点击头像在浏览器中直接跳转到对应 GitHub 个人主页，并内置离线兜底缓存，确保断网或 API 限流时依然稳定美观展示。
