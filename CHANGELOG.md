...
### CAD 专业软件专属按键唤醒与双轨智能路由

1. **解决 CAD 原生笔势冲突痛点 (CAD Mouse Gestures Harmony)**
   - **黑名单专属触发按键映射**：在黑名单（排除名单）中，允许为特定程序（如 `sldworks.exe`, `autocad.exe`, `inventor.exe` 等）单独配置专属呼出按键（如鼠标中键、侧键或 `Alt + 右键` 等组合键）。
   - **零时延放行原生右键**：在配置了专属按键的黑名单程序中，普通鼠标右键 **100% 绝对零时延放行**给宿主程序使用其原生 CAD 笔势，StarPie 绝不拦截或干扰。
   - **专属唤醒精准触发**：按下为该程序配置的专属按键时，精准唤出 StarPie 轮盘；当切换至普通应用程序或桌面时，自动无缝恢复全局默认触发键（如右键），彻底打破 CAD 原生笔势与全局手势轮盘的二者冲突。

2. **Tab 3 进程列表与专属按键交互重构**
   - **专属按键状态可视化徽章**：黑名单列表中每一项均清晰展示该进程当前的唤醒键状态（如未配置的 `🚫 未配置 (完全放行右键)`，或已配置的 `🎯 鼠标中键` / `🎯 鼠标侧键 1` 绿色徽章）。
   - **完全复刻物理录制卡片**：复刻 Tab 2 轮盘唤醒键录制交互体验，配备 `[ 🔴 点击录制专属按键 / 组合键 ]`、`[ 🔄 恢复默认 (完全放行) ]` 与硬件感知器实时反馈，支持鼠标任意按键、键盘单键及修饰组合键物理录制与 ESC 极速取消。
   - **行内快捷操作**：黑名单列表中支持一键 `[ ⚙️ 配置触发键 ]` 直接进入监听录制，`[ 🔄 恢复放行 ]` 清除专属按键，以及 `[ 🗑️ ]` 移除进程。

### 全盘秒搜 (Quick Search) 体验优化与尺寸自由拖拽

1. **新增「🎬 视频」独立分类过滤**
   - 分类过滤芯片增加 `🎬 视频` 选项，内置涵盖常见视频格式（`.mp4`, `.mkv`, `.avi`, `.mov`, `.flv`, `.wmv`, `.rmvb`, `.webm`, `.ts`, `.m4v`, `.3gp`, `.f4v` 等）。
   - 无论是底层原生深度搜索引擎还是 Everything IPC 加速，均实现毫秒级精准过滤；未输入检索词时自动根据使用习惯推荐近期视频与素材文件。

2. **窗口置顶图钉锁定 (Pin Window)**
   - 右上角新增 `📌` 图钉按钮，点击一键切换置顶常驻状态；
   - 处于置顶状态时，窗口即使失去焦点（点击外部桌面或其他应用）也不会自动关闭隐藏，方便多屏对比、边搜边写或工程对照。

3. **自由拖拽拉伸窗口尺寸与持久化记忆**
   - 右下角新增专业点阵手柄（`WindowResizeGrip`），并覆盖右侧与底侧边缘拉伸响应区，支持用户随心所欲自由调整窗口宽度与高度（最小宽度 560px，最小高度 380px）。
   - 调整后的尺寸自动保存至用户配置，下一次呼出时完美保持偏好的窗口大小。
