...
StarPie v1.7.4-beta.2 专项优化了全盘秒搜（Quick Finder）与第三方 Everything 的底层协同机制，解决了 Everything 以管理员权限运行时受 Windows UIPI 安全隔离导致的通信受阻与延迟问题，增加了底栏状态动态交互徽章与一键提权/启动引导，并为内置原生并发引擎引入了高频热点目录极速优先扫描层。

### 🌟 核心改进与新增功能

1. **Everything 权限受阻底层精确感知与一键管理员提权**
   - **痛点根因定位**：当本地 Everything 配置以管理员权限运行（`run_as_admin=1`）时，Windows UIPI（用户界面特权隔离）安全机制会阻断标准用户权限的 StarPie 通过 `WM_COPYDATA` 与 Everything 建立 IPC 通信（返回错误码 2 `EVERYTHING_ERROR_IPC`），导致秒搜静默回退至全盘深层目录遍历。
   - **智能状态感知**：`EverythingService` 现已支持精准区分并捕获 4 种连通状态：`EverythingConnected`（直连 0ms）、`EverythingPermissionBlocked`（权限受阻）、`EverythingNotRunning`（未运行）与 `NativeOnly`（纯原生）。
   - **一键管理员提权重启**：当检测到受阻时，底栏徽章与空结果卡片提供醒目的「🛡️ 以管理员身份重启 StarPie」操作引导，确认后直接调用提权机制重启 StarPie，与管理员权限 Everything 建立无缝 IPC 通信，实现 0ms 极速秒搜。

2. **本地 Everything 智能路径探测与一键拉起**
   - **多路径全自动识别**：覆盖桌面快捷方式/便携版、AppData 漫游目录、标准 Program Files 路径、注册表 App Paths 及开始菜单。
   - **一键拉起与热连接**：当 Everything 未在后台运行时，底栏状态徽章显示「🐢 原生并发 (点击启动 Everything)」，点击即可一键启动本地 Everything 并自动建立索引数据库连接。

3. **空搜索结果交互引导重构**
   - **告别无反馈盲区**：搜索未匹配到结果时，不再仅展示通用占位符，而是根据当前检索引擎状态动态呈现对应的解决操作（如管理员提权按钮或拉起 Everything 按钮），操作指引更清晰明确。

4. **内置原生引擎高频热点目录极速优先扫描**
   - **热点目录优先层**：在发起全盘深盘（最高 15 层）广度优先遍历之前，优先流式枚举桌面、下载、文档与常用工程目录中的文件。
   - **响应提速**：搜索桌面或下载目录中的文件时，原生引擎响应时间由数秒缩短至 5ms 以内，大幅提升了无 Everything 环境下的原生检索体验。

5. **检索引擎动态状态徽章**
   - **实时反馈与 ToolTip 指引**：搜索窗口底栏新增检索引擎状态指示徽章，用不同语义颜色（绿色极速 / 黄色提权受阻 / 橙色未运行 / 灰色原生）直观展示当前后端，悬停可查看详细状态说明，点击可快速触发相应动作。
