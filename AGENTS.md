...
| **v1.7.3-beta.4** | 2026-09-09 | 内存按需轻量驻留深度优化 & 静默更新自愈守护：任务栏状态预取 (WindowTaskbarHelper) 按需调度，仅在轮盘方案包含 SwitchWindow/Taskbar/Tile 时启动 UIAutomation 预取，规避常规手势下线程与句柄积压；自动更新脚本与管理员提权重启显式追加 --silent 参数，修复更新重启后误开控制台导致内存冲上 160MB+ 缺陷；旧版配置 Base64 嵌入数据启动自动洗涤并持久化；控制台 30 秒延迟完全销毁后调度强制工作集规整，让内存迅速利落回落至 15MB~30MB 极致轻量后台基准。 |
