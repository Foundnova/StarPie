...
| **v1.6.8** | 2026-09-04 | 视觉品牌与交互大成：全新星盘宇宙公转轨道与四芒星核超清图标 (cover.v3)、蜂窝扇二级轮盘方位颠倒修复、控制台一二级配置模式双向联动与实时预览保持、轮盘扇区自适应弹性字号 (Auto Font-Fit)、语言选单微标规范化、视口呼吸留白与高对比度滑块、全量多语言覆盖、画布缩放修复、快捷键Pause与搜索、独占暂停全局热键、侧边栏主题切换、贡献者致谢离线策略、平铺设置折叠、深色对比度优化、扇区文字位置与微调、屏幕边缘防溢出 |
