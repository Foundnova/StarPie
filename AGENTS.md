...
| **v1.6.9** | 2026-09-06 | 继承图标优先级与持久化守卫 & 二级菜单容量匹配：程序图标继承渲染优先级提升与平铺子模式覆写保护、开机秒启懒加载与退出配置无损持久化守卫彻底根除防误触被关闭、六大画布与轮盘渲染通路图标继承逻辑全面对齐、动作聚焦编辑卡片增加关联图标徽标与预览、二级菜单蜂窝扇(最多3项)与外圈子环(最多4项)说明描述与添加/渲染动态容量全闭环匹配 |
