...
| **v1.7.3-beta.4** | 2026-09-09 | 内存按需轻量驻留深度优化 & 透明轮盘 HWND 复用：任务栏状态预取 (WindowTaskbarHelper) 按需调度，仅在轮盘方案包含 SwitchWindow/Taskbar/Tile 时启动 UIAutomation；自动更新与管理员提权重启追加 --silent，旧版配置 Base64 数据自动洗涤；控制台延迟销毁后强制工作集规整；改为进程级复用 RadialWindow/HWND，只重建视觉树，不创建或销毁实例。 |
