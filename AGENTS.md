...
| **v1.7.3-beta.4** | 2026-09-09 | 4–12 任意自定义轮盘方位数量支持 & 任务栏/托盘右键原生透传守护 (根治 Issue #92) & 内存按需轻量驻留与透明轮盘单 HWND 复用：打破固定 4/8/12 扇区硬编码限制，支持 4~12 任意自定义扇区数量（如 6 键手势等）并修复扇区角度插值公式；引入 IsPointOnTaskbar 物理坐标判定，光标位于任务栏或托盘时绝对透传物理按键事件，设置窗口隐藏后台显式注销 ShowInTaskbar，彻底根除任务栏固定图标右键 JumpList 失灵；任务栏状态预取按需调度，更新与提权追加 --silent，旧配置 Base64 自动洗涤；进程级复用透明轮盘 HWND，手势结束仅隐藏，配置修订时才重建视觉树。 |
