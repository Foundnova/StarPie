...
| **v1.7.3-beta.5** | 2026-09-10 | 唤出一二级轮盘全展开 & 开箱高颜值默认配置友好优化 & 二级级联动作添加按钮偶发禁用根除 & 非全局方案子盘继承与就地物化：外圈同心子环二级菜单支持唤出时直接同时展开全部一二级轮盘；在「多级轮盘与级联子菜单」卡片中新增该功能配置开关；支持子扇区极速直选与主副扇区动态联动高亮，平滑入场无掉帧；全面重塑新手开箱默认配置，默认启用液态毛玻璃 (Glassmorphism) + 浅色模式 (Light) + 纯图标居中 (IconOnly) 高质感科技湛蓝配色，四象限预设实用子动作，开箱即美；彻底根治功能配置界面二级动作添加按钮偶发禁用缺陷；修复专属程序方案下二级子环屏幕覆盖却无法触发问题，实现画布与手势精准判定及就地无缝物化编辑。 |
