...
3. **肌肉记忆与确定性 (Muscle Memory & Reliability)**：
   - 扇区角度与操作严格绑定空间极坐标方向（4 字键、8 字键、12 字键预设档，及 4–12 键任意自定义档，均为 360°/N 等分且第 0 位固定正东）；
...
- **一级轮盘与外圈子环 (Outer Sub-Ring)**：
  根据扇区数量 $N \in [4, 12]$（预设档 4/8/12 与自定义档 5/6/7/9/10/11，即 4–12 键任意整数）均匀划分扇区角度 $\Delta \theta = 2\pi / N$，扇区中心角 $\theta_i = -\pi/2 + i \cdot \Delta \theta$。
- **蜂窝扇 (Honeycomb Fan)**：
...
### 6. 📜 版本里程碑演进图谱 (v1.0.0 ~ v1.7.3-beta.4)
...
| **v1.7.3-beta.4** | 2026-09-09 | 4–12 任意自定义轮盘方位数量支持 & 任务栏/托盘右键原生透传守护 (根治 Issue #92) & 内存按需轻量驻留与透明轮盘单 HWND 复用防闪烁：打破固定 4/8/12 扇区硬编码限制，支持 4~12 任意自定义扇区数量（如 6 键手势等）并修复扇区角度插值公式；引入 IsPointOnTaskbar 物理坐标判定，光标位于任务栏或托盘时绝对透传物理按键事件，设置窗口隐藏后台显式注销 ShowInTaskbar，彻底根除任务栏固定图标右键 JumpList 失灵；任务栏状态预取按需调度，更新与提权追加 --silent，旧配置 Base64 自动洗涤；进程级复用透明轮盘 HWND，手势结束隐藏内容层常驻 HWND 防 DWM 闪烁，引入 _isPresented 生命周期守卫与单次 Render 调度消除 32ms 冗余延迟并彻底根除幽灵轮盘。 |
